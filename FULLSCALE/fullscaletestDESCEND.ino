...
// Altitude band
#define ALT_BAND_FT_LO  15.0f
#define ALT_BAND_FT_HI  25.0f
#define METERS_PER_FT   0.3048f

// Servo (home 40°, release 90°)
#define SERVO_PIN       2
...
#define SERVO_ANGLE_MIN   -60 // ignore, just a precaution
...
// descent check
static float last_alt_ft = 0.0f;           // previous altitude (ft)
static bool  triggered_this_descent = false;  // clear when leaving band
...
// altitude band 
static bool inAltitudeBand(float alt_m) {
  float alt_ft = alt_m / METERS_PER_FT;
  return (alt_ft >= ALT_BAND_FT_LO && alt_ft <= ALT_BAND_FT_HI);
}

// servo stuff (home 40°, release 90°)
static uint32_t servoPeriod_us(void) {
  return 1000000UL / SERVO_PWM_FREQ;
}

static uint32_t servoUsToDuty(uint32_t pulse_us) {
  uint32_t maxDuty = (1UL << SERVO_PWM_RES) - 1UL;
  uint32_t period = servoPeriod_us();
  if (pulse_us > period) pulse_us = period;
  return (pulse_us * maxDuty) / period;
}

static void writeServoPulse(uint32_t pulse_us) {
  ledcWrite(SERVO_PIN, servoUsToDuty(pulse_us));
}

static void writeServoAngle(int angle) {
  if (angle != 40 && angle != 90) return;
  long pulse = map(angle, SERVO_ANGLE_MIN, SERVO_ANGLE_MAX,
                   SERVO_PULSE_MIN_US, SERVO_PULSE_MAX_US);
  writeServoPulse(pulse);
}

static void runServoRelease(void) {
...
void enterLightSleep(void) {
  ...
  servo_ok = ledcAttach(SERVO_PIN, SERVO_PWM_FREQ, SERVO_PWM_RES);
  if (servo_ok) writeServoAngle(40);
...
void setup() {
  ...
  servo_ok = ledcAttach(SERVO_PIN, SERVO_PWM_FREQ, SERVO_PWM_RES);
  if (servo_ok) writeServoAngle(40);  // start at home
...
void loop() {
  ...
  // trigger servo when descending through band
  ...
  if (in_band && descending && !triggered_this_descent && servo_ok) {
    runServoRelease();
    triggered_this_descent = true;
  }
  if (!in_band) triggered_this_descent = false;  // allow next descent through band
  last_alt_ft = alt_ft;
