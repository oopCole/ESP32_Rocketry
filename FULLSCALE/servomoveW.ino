...
#define DELAY_AFTER_SERVO_MS   2000   // delay before soil sensor
#define SOIL_SENSOR_DURATION_MS 5000  // soil sensor reading
#define DELAY_BEFORE_SLEEP_MS  2000   // delay before deep sleep
...
// Soil sensor reads for ~5 seconds (EC, pH, NO3-N)
static void runSoilSensorSequence() {
  const uint32_t startMs = millis();
  while (millis() - startMs < SOIL_SENSOR_DURATION_MS) {
    uint8_t result = node.readHoldingRegisters(0x0002, 3);
    if (result == node.ku8MBSuccess) {
      float ec = (float)node.getResponseBuffer(0);
      float ph = node.getResponseBuffer(1) * 0.1f;
      float n  = (float)node.getResponseBuffer(2);
      Serial.print(" EC: "); Serial.print(ec, 0); Serial.println(" uS/cm");
      Serial.print(" pH: "); Serial.println(ph, 1);
      Serial.print("  N: "); Serial.print(n, 0); Serial.println(" mg/kg");
      Serial.println("===============");
    } else {
      Serial.print("=== SENSOR FAILURE === Error 0x"); Serial.println(result, HEX);
    }
...
void loop() {
  ...
  // soil sensing (5s read)
  delay(DELAY_AFTER_SERVO_MS);
  runSoilSensorSequence();

  // deep cleep
  delay(DELAY_BEFORE_SLEEP_MS);
  go_to_sleep();
}
